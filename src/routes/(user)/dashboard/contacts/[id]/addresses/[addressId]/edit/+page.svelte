<script>
  ...
  async function handleSubmit(e) {
    ...
    const response = await addressUpdate(token, contactId, addressId, address);
  ...
  async function fetchAddressDetail() {
    const response = await addressDetail(token, contactId, addressId);
    const resBody = await response.json();

    if (response.status === 200) {
      address = resBody.data;
    } else {
      await alertError(resBody.errors);
    }
  }
  ...
  async function fetchContactDetail() {
    const response = await contactDetail(token, contactId);
    const resBody = await response.json();
    ...
    <form onsubmit={handleSubmit}>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        <div>
          <label
            for="country"
            class="block text-gray-300 text-sm font-medium mb-2">Country</label
          >
          <div class="relative">
            <div
              class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"
