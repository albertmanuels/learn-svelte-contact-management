  ...
  import { userRegister } from "$lib/api/UserApi";
    ...
    async function handleSubmit(e) {
        ...
        if(response.status === 200) {
           ...
           await alertSuccess("Success register user")
           await goto("/login")
        } else {
...
<div class="animate-fade-in bg-gray-800 bg-opacity-80 p-8 rounded-xl shadow-custom border border-gray-700 backdrop-blur-sm w-full max-w-md">
    ...
    <form onsubmit={handleSubmit}>
        ...
        <div class="mb-6">
            ...
            <div class="relative">
                ...
                <input type="password" id="confirm_password" name="confirm_password" 
                    class="w-full pl-10 pr-3 py-3 bg-gray-700 bg-opacity-50 border border-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" 
                    placeholder="Confirm your password" required bind:value={user.confirmPassword}>
            </div>
        </div>
        ...
        <div class="text-center text-sm text-gray-400">
            ...
            <a href="/login" class="text-blue-400 hover:text-blue-300 font-medium transition-colors duration-200">Sign in</a>
        </div>
    </form>
</div>
